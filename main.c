...
int main(){
    ...
    while (!WindowShouldClose()){
        ...
        if (gamestate == STATE_MENU)
        {
            ...
            draw_ground();
            menu();
            if (inputPressed) gamestate = STATE_PLAYING;
        }
...
void move_pipe(Pipe pipes[], int total_pipes, float dt){
    ...
    if (move != -1){
        ...
        pipes[move].y = GetRandomValue(50, HEIGHT - ground.height*scale.ground.y - 60 - pipe_vertical_distance);
        pipes[move].passed = 0;
    }
}
...
void draw_score(int score) {
    ...
    float digit_scale = 1.5f;
    float total_width = 0.0f; 

    for (int i = count - 1; i >= 0; i--) {
        int digit = digits[i];
        total_width += numbers[digit].width * digit_scale;
    }
...
int check_death(float pos_x, float pos_y, Pipe pipes[], int total_pipes){
    ...
    for  (int i=0; i < total_pipes; i++){
        int within_pipe = (pipes[i].x <= pos_x + bird_width/2 && pipes[i].x + green_pipe.width*scale.pipe.x >= pos_x - bird_width/2);
        #if HITBOX
            DrawRectangle(pipes[i].x, pipes[i].y, green_pipe.width*scale.pipe.x, pipe_vertical_distance, (Color){0, 100, 0, 50}); 
        #endif
...
void draw_game_over(void){
    float game_over_scale = 2;
    DrawTexturePro(
        game_over_text,
        (Rectangle){0.0f, 0.0f, game_over_text.width, game_over_text.height},
        (Rectangle){WIDTH/2,game_over_height, game_over_text.width*game_over_scale, game_over_text.height*game_over_scale},
